#!/usr/bin/env python3
"""
Arctic Species Trade Data Pipeline
A comprehensive system to extract and process trade data for Arctic species
"""
...
class ArcticSpeciesTradeAnalyzer:
    def __init__(self):
    ...
    def normalize_species_name(self, common_name: str) -> Tuple[str, Optional[str]]:
        """Normalize common name and get scientific name if available"""
        # Clean up the name
        normalized = common_name.strip()
        
        # Check if we have a mapping
        scientific_name = self.name_mapping.get(normalized)
        
        return normalized, scientific_name
        
    def get_species_id(self, species_name: str) -> Optional[str]:
        """Query database for species ID using common or scientific name"""
        try:
            normalized_name, scientific_name = self.normalize_species_name(species_name)
            ...
            # If not found and we have scientific name, try that
            if scientific_name:
            ...
            logger.warning(f"Species not found in database: {species_name}")
            return None
    ...
    def get_trade_summary(self, species_id: str) -> Optional[Dict]:
        """Get pre-computed trade summary from species_trade_summary table"""
        try:
            response = self.supabase.table('species_trade_summary').select('*').eq('species_id', species_id).execute()
            
            if response.data and len(response.data) > 0:
                # Aggregate data from all summary records
                summaries = response.data
                
                total_records = sum(s.get('total_records', 0) for s in summaries)
                total_quantity = sum(s.get('total_quantity', 0) or 0 for s in summaries)
                
                # Get overall min/max years
                min_years = [s.get('min_year') for s in summaries if s.get('min_year')]
                max_years = [s.get('max_year') for s in summaries if s.get('max_year')]
                
                return {
                    'total_trade_records': total_records,
                    'overall_total_quantity': total_quantity,
                    'overall_min_year': min(min_years) if min_years else None,
                    'overall_max_year': max(max_years) if max_years else None
                }
            else:
                # Fallback: query trade records directly if no summary exists
                logger.info(f"No trade summary found for species {species_id}, querying trade records directly")
                
                # Get count and sum from trade records
                ...
                # Get quantities and years
                quantity_response = self.supabase.table('cites_trade_records').select('quantity, year').eq('species_id', species_id).execute()
                
                if quantity_response.data:
                    total_quantity = sum(r.get('quantity', 0) or 0 for r in quantity_response.data)
                    years = [r.get('year') for r in quantity_response.data if r.get('year')]
            ...
            logger.error(f"Error getting trade summary for species {species_id}: {str(e)}")
            return None
    ...
    def get_conservation_status(self, species_id: str) -> Dict:
        """Get IUCN and CITES status with change detection"""
        try:
            result = {
                "iucn_status": "",
            ...
            # Get CITES status
            cites_response = self.supabase.table('cites_listings').select('appendix').eq('species_id', species_id).order('effective_date', desc=True).limit(2).execute()
    ...
    def process_species(self, species_name: str) -> Dict:
        ...
        try:
            ...
            # Step 2: Get trade summary data
            trade_summary = self.get_trade_summary(species_id)
            if trade_summary:
            ...
            # Step 7: Get conservation status
    ...
    def export_to_csv(self, df: pd.DataFrame, filename: str = "arctic_species_trade_completed.csv"):
...
if __name__ == "__main__":
    try:
        analyzer = ArcticSpeciesTradeAnalyzer()
        report = analyzer.generate_report()
        analyzer.export_to_csv(report)
        print("Trade data extraction completed successfully!")
    except Exception as e:
